...
function before ($dis, $inthat)
{
	return substr($inthat, 0, strpos($inthat, $dis));
}
function between($dis, $that, $inthat)
{
	return before ($that, after($dis, $inthat));
}
function after ($dis, $inthat)
{
	if (!is_bool(strpos($inthat, $dis)))
	return substr($inthat, strpos($inthat,$dis)+strlen($dis));
}
